...
export default function useCollaboration(filePath, sessionId, onLocalUpdate, onRemoteSave) {
  ...
  const emitSaved = () => {
    socketRef.current?.emit("file-saved", { filePath });
  };

  return { ydoc: ydocRef.current, ytext: ytextRef.current, isSynced, emitSaved, collabCount };
}
